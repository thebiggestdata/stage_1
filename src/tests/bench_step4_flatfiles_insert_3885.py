...
def main():
    ...
    csv_path = OUTDIR / "step4_books_3885.csv"
    jsonl_path = OUTDIR / "step4_books_3885.jsonl"
    if csv_path.exists(): csv_path.unlink()
    if jsonl_path.exists(): jsonl_path.unlink()

    t_csv = write_csv(rows, csv_path)
    t_json = write_jsonl(rows, jsonl_path)

    print(f"INSERT_3885_CSV_SECONDS={t_csv:.2f}")
    print(f"INSERT_3885_JSON_SECONDS={t_json:.2f}")
